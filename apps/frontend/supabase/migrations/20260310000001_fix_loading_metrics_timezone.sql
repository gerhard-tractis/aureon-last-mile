-- Fix: All loading metrics functions must use America/Santiago timezone
-- so that created_at filters and DATE() groupings align with Chilean business days.

-- 1. Package loading statistics
...
-- 3. Committed orders per delivery date (uses delivery_date, not created_at — no timezone issue)
-- Recreate for consistency with SET timezone
