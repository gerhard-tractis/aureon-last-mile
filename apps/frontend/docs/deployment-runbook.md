...
3. `20260911000003_spec79_b1_withdraw_vehicle_per_day_index.sql` — **drops** `routes_one_vehicle_per_day`. Spec-79 B-1 (review round 6) found the index contradicted the spec's own Fase 0 finding 3 ("un camión puede legítimamente correr dos rutas el mismo día"): once a truck's second-turn route hit the constraint, every "Reintentar" press created one more orphaned route in DispatchTrack, unbounded. `PATCH /api/dispatch/routes/[id]`'s own `23505` special-case (which assumed the index existed) was removed in the same change — deploy (3) together with the frontend bundle that removes that branch, not separately.

   **Correction (spec-79 M6, review round 7): the window between (2) applied and (3) applied is NOT harmless.** It used to be described that way here — wrong. While `routes_one_vehicle_per_day` exists and the OLD frontend bundle (still expecting the `23505` branch) is running against it, B-1's own unbounded orphan-DT-route loop (described in this migration's own header) is **fully live**: a truck's legitimate second-turn route hits the index, `dispatch-local-completion.ts`'s persist write 23505s, `external_route_id` never gets stored, and every "Reintentar" press creates one more route in DispatchTrack with no ceiling. The only pairing that IS harmless is the reverse — an OLD frontend still expecting the `23505` branch running AFTER (3) has already dropped the index simply falls through to a generic 500, not a duplicate route. Deploy (3) with (or before) the frontend bundle that removes the `23505` branch; never deploy (2) without (3) queued immediately behind it.
...
2. `infra/supabase-qa/systemd/aureon-dt-mock-qa.service` — the intended long-term way to run it (survives reboots). **Installed and enabled** (2026-09-05) as `aureon-dt-mock-qa`, running as `aureon` on `127.0.0.1:4477`, `Restart=always` so it comes back after a crash, a SIGTERM, or a reboot. Verify with `systemctl is-enabled --quiet aureon-dt-mock-qa && systemctl is-active aureon-dt-mock-qa`. It was previously **not installed** — this task's sandbox could not write to `/etc/systemd/system` or call `systemctl daemon-reload`/`enable`. Until someone with interactive VPS access installs it, the mock runs as a plain background process, **started as the `aureon` user** — it does **not** survive a VPS reboot. Check before relying on it:
   ```bash
