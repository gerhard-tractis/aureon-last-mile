...
export function PickupFlowHeader({ loadId, scanned, total }: PickupFlowHeaderProps) {
  ...
  return (
    <div className="bg-accent text-accent-foreground dark:bg-accent-muted dark:text-accent p-4 -mx-4 -mt-4 mb-4">
      <p className="text-xs opacity-80">{loadId}</p>
      <p className="font-mono text-xl font-bold">{scanned} / {total}</p>
      <div className="bg-white/20 dark:bg-white/10 rounded h-1.5 mt-2">
        <div
          role="progressbar"
          aria-valuenow={scanned}
          aria-valuemin={0}
          aria-valuemax={total}
          className="bg-white dark:bg-accent h-1.5 rounded transition-all"
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
