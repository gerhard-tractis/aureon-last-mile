...
describe.skipIf(skip)('comunas normalization (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    const { data } = await supabase.from('chile_comunas').select('id').limit(1);
    hasData = (data?.length ?? 0) > 0;
  });

  it('normalize_comuna_id returns UUID for exact canonical name', async (ctx) => {
    if (!hasData) ctx.skip();
  ...
  it('normalize_comuna_id is case-insensitive for canonical name', async (ctx) => {
    if (!hasData) ctx.skip();
  ...
  it('normalize_comuna_id resolves uppercase alias', async (ctx) => {
    if (!hasData) ctx.skip();
  ...
  it('normalize_comuna_id resolves accentless alias NUNOA → Ñuñoa', async (ctx) => {
    if (!hasData) ctx.skip();
  ...
  it('normalize_comuna_id returns null for unknown commune', async (ctx) => {
    if (!hasData) ctx.skip();
    const { data } = await supabase.rpc('normalize_comuna_id', { raw_name: 'Antartica Chilena XYZ' });
    expect(data).toBeNull();
  });

  it('normalize_comuna_id returns null for empty string', async (ctx) => {
    if (!hasData) ctx.skip();
