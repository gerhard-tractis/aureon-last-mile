            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={8} className={TD_EMPTY}>
                  {activeFilters > 0 ? 'Sin resultados para los filtros seleccionados' : 'Sin órdenes en esta etapa'}
                </td>
              </tr>
            ) : (
              filtered.map((o) => {
...
/** Compute KPIs from an order list — shared across all stage panels */
export function computeOrderKpis(orders: OrderRow[]) {
  type Pkg = { id: string };
  const clients = new Set(orders.map((o) => o['retailer_name'] as string).filter(Boolean));
  const points = new Set(orders.map((o) => o['pickup_point_name'] as string).filter(Boolean));
  const totalBultos = orders.reduce((sum, o) => sum + ((o['packages'] as Pkg[])?.length ?? 0), 0);
  return [
    { label: 'Órdenes', value: String(orders.length) },
    { label: 'Bultos', value: String(totalBultos) },
    { label: 'Clientes', value: String(clients.size) },
    { label: 'Puntos pickup', value: String(points.size) },
  ];
}
