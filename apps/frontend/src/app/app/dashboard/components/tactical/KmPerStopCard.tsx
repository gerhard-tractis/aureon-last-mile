...
interface KmPerStopCardProps {
  data: RouteTacticsData | null;
  isLoading: boolean;
}

export function KmPerStopCard({ data, isLoading }: KmPerStopCardProps) {
  if (isLoading) {
    return (
  ...
  const value = formatNumber(data?.avg_km_per_stop ?? null);
  ...
  return (
    <div className="rounded-lg border bg-card p-4 flex flex-col gap-2">
      ...
      <span className="text-xs text-muted-foreground">
        Kilómetros promedio entre paradas
      </span>
    </div>
  );
}
