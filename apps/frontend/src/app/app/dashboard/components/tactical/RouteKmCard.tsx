...
interface RouteKmCardProps {
  data: RouteTacticsData | null;
  isLoading: boolean;
}

export function RouteKmCard({ data, isLoading }: RouteKmCardProps) {
  if (isLoading) {
    return (
  ...
  const value = formatNumber(data?.avg_km_per_route ?? null);
