...
describe('claimDispatchAttempt', () => {
  it('claims fresh when dispatch_attempt_at is NULL — wasStale is false', async () => {
    ...
    expect(result).toEqual({ claimed: true, wasStale: false });
  ...
  it('reclaims a stale attempt (older than DISPATCH_CLAIM_STALE_MS) — wasStale is true', async () => {
    ...
    expect(result).toEqual({ claimed: true, wasStale: true });
...
describe('releaseDispatchClaim', () => {
  it('sets dispatch_attempt_at back to NULL, scoped by id and operator_id', async () => {
    const eqOperatorMock = vi.fn().mockResolvedValue({ error: null });
    const eqIdMock = vi.fn().mockReturnValue({ eq: eqOperatorMock });
    const updateMock = vi.fn().mockReturnValue({ eq: eqIdMock });
    const supabase = { from: vi.fn().mockReturnValue({ update: updateMock }) };

    await releaseDispatchClaim(supabase as never, { routeId: 'r1', operatorId: 'op-1' });
  ...
  it('logs but does not throw when the release write fails — best-effort, like every other release in this flow', async () => {
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const eqOperatorMock = vi.fn().mockResolvedValue({ error: { message: 'db down' } });
    const eqIdMock = vi.fn().mockReturnValue({ eq: eqOperatorMock });
    const updateMock = vi.fn().mockReturnValue({ eq: eqIdMock });
    const supabase = { from: vi.fn().mockReturnValue({ update: updateMock }) };

    await expect(
      releaseDispatchClaim(supabase as never, { routeId: 'r1', operatorId: 'op-1' }),
   ...
   * into an unrelated 502 DT_API_ERROR via the handler's own outer catch.
   */
