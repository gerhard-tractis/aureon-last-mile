/**
 * Chilean administrative divisions data.
 * Used for comuna validation during order import.
 * CANONICAL source - reused by Stories 2.3, 2.4.
 */

export const REGIONS: Record<string, { name: string; comunas: string[] }> = {
  'I': {
    name: 'Tarapacá',
...
export const ALL_COMUNAS: string[] = Object.values(REGIONS).flatMap((r) => r.comunas);

// Build a Set with normalized (lowercase, trimmed) comuna names for fast lookup
const COMUNAS_SET = new Set(ALL_COMUNAS.map((c) => c.toLowerCase().trim()));

export function isValidComuna(comuna: string): boolean {
  return COMUNAS_SET.has(comuna.toLowerCase().trim());
}
