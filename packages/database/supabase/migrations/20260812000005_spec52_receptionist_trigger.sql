...
-- removes none: it deliberately does NOT drop close_pickup_route(), does NOT
-- remove the '-> in_transit' branch of trg_pickup_routes_status_sync, and does
-- NOT add uniq_pickup_routes_one_active_per_vehicle — all three belong to
-- Task 8. The database ships ahead of the frontend: drivers still see the
-- "close route" button, and the reception UI still gates on
-- status = 'in_transit', reached today ONLY through close. Removing either
-- side now would deadlock production — no route could reach reception at all.
...
--     later receptionist scan finds that live batch at STEP 2 and returns it
--     untouched — a pure no-op.
--   * receptionist scans first -> open_route_reception inserts the batch
--     BEFORE flipping the route status, so by the time the trigger fires its
--     ON CONFLICT DO NOTHING has nothing to do. Statement order is
--     load-bearing; do not move STEP 4 after STEP 6.
-- =============================================================================
...
-- BEFORE being attached to a route — the frontend inserts a pickup_scans row
-- on every scan, and pickup_route_id is NULL then. `IF v_status IS DISTINCT
-- FROM 'in_progress'` looks equivalent and is not: NULL IS DISTINCT FROM
-- 'in_progress' is TRUE, so it rejects the whole ordinary pickup flow.
...
AS $$
DECLARE
  v_status public.pickup_route_status_enum;
  v_code   TEXT;
BEGIN
...
BEGIN
  ...
  IF v_route.status = 'received' THEN
    RAISE EXCEPTION 'La ruta % ya fue recibida en el hub', v_route.code
      USING ERRCODE = '55000';
  END IF;
  IF v_route.status = 'cancelled' THEN
    RAISE EXCEPTION 'La ruta % fue anulada y no puede recibirse', v_route.code
      USING ERRCODE = '55000';
  END IF;
  ...
  -- re-stamping in_transit_at rewrites history, recomputing expected_count
  -- moves the number the hub counts against. `deleted_at IS NULL` is
  -- mandatory — reopen_pickup_route soft-deletes the batch, and without the
  -- filter a reopen-then-rescan hands back the dead one (regression test:
  -- spec52_reopen_route.sql CASE 2).
  SELECT * INTO v_rr
...
COMMENT ON FUNCTION public.open_route_reception(UUID)
  IS 'Receptionist QR scan: open (or return) the consolidated reception batch for a pickup route, flip it to in_transit and set its manifests to awaiting_reception (spec-52). Idempotent, and a no-op on a route the driver already closed via close_pickup_route.';
...
BEGIN
  ...
  IF v_rr.id IS NOT NULL THEN
    SELECT COUNT(*) INTO v_scans
      FROM public.reception_scans
     WHERE reception_id = v_rr.id;
    IF v_scans > 0 THEN
      RAISE EXCEPTION
  ...
  SELECT code INTO v_blocker
    FROM public.pickup_routes
   WHERE operator_id = v_operator
     AND driver_id   = v_route.driver_id
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
     WHERE n.nspname = 'public' AND p.proname = 'close_pickup_route'
  ) THEN
  ...
  IF (SELECT prosrc FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
       WHERE n.nspname = 'public'
         AND p.proname = 'trg_pickup_routes_set_manifest_reception_status')
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
     WHERE tgrelid = 'public.pickup_scans'::regclass
       AND tgname = 'trg_pickup_scans_route_lock'
  ) THEN
