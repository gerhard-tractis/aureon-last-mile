-- =============================================================================
-- spec-80 fase 1b — close_manifest: ACL heredado sin revocar + centinelas
-- =============================================================================
-- Fase 1 (20260913000002) hizo `GRANT EXECUTE ... TO authenticated` sin
-- ningún REVOKE. El proacl real que queda es
-- {=X/postgres, postgres=X, anon=X, authenticated=X, service_role=X}:
--
--   - `=X/postgres` es PUBLIC con EXECUTE (el ACL implícito por defecto de
--     Postgres en toda función nueva, nunca revocado).
--   - `anon=X` entra por default privileges de Supabase — se concede
--     directamente A anon, no se hereda de PUBLIC, así que
--     `REVOKE ... FROM PUBLIC` a secas no lo quita.
--
-- Repo convention (20260812000003:298, 20260812000005:273,
-- 20260820000003:316, 20260820000005:110), y su precedente más reciente:
-- spec-85 fase 2 (20260913000003) hizo exactamente este mismo arreglo en sus
-- tres RPCs tras un hallazgo de review — usada aquí como plantilla.
--
-- No hay fuga explotable hoy: el guard `v_operator IS NULL` corta a `anon`
-- con un 42501 antes de que el ACL importe. Esto es defensa en profundidad y
-- coherencia con la convención del repo, no el cierre de un agujero abierto.
--
-- De paso: los dos RAISE de 42501 (v_operator IS NULL, manifest not found)
-- no llevaban prefijo centinela — el resto de la función ya lo usa
-- (MANIFEST_NOT_CLOSABLE, MANIFEST_ALREADY_SIGNED, OPERATOR_SIGNATURE_REQUIRED),
-- y lib/pickup/closeManifestErrors.ts en el frontend mapea por ese prefijo.
-- Sin él, esas dos causas eran indistinguibles de cualquier otro 42501 para
-- ese mapeador — caían al genérico igual que ahora (son anomalías que el
-- operador no puede accionar, no un rechazo de negocio que explicar), pero
-- por accidente, no por contrato. NO_OPERATOR_IN_JWT y MANIFEST_NOT_FOUND
-- son los mismos nombres que record_discrepancies (20260913000003) usa para
-- las causas equivalentes.
--
-- CREATE OR REPLACE preserva el proacl existente — por eso el ACL de fase 1
-- sigue vivo hoy pese a que esta migración reemplaza el cuerpo. Los REVOKE de
-- abajo son necesarios incluso reemplazando la función entera.
-- =============================================================================
...
'spec-80 fase 1b. Atomically closes a pickup manifest: status=completed,
...
such. Rejects a manifest from another operator (42501,
"MANIFEST_NOT_FOUND: ..."), no operator_id in the JWT (42501,
"NO_OPERATOR_IN_JWT: ..."), one in a non-closable status (P0001,
...
-- fase 1b: fase 1 dejó sólo el GRANT de abajo, sin ningún REVOKE — el proacl
-- real quedaba con PUBLIC y anon con EXECUTE (ver cabecera de este archivo).
-- CREATE OR REPLACE preserva el ACL existente, así que estos REVOKE son
-- necesarios pese a reemplazar la función entera arriba.
...
BEGIN
  ...
  RAISE NOTICE '✓ spec-80 fase 1b close_manifest ACL fix migration complete';
END $$;
