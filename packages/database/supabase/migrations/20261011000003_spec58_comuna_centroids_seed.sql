-- spec-58 fase 2: comuna centroids on `public.chile_comunas` -- SEED ONLY
-- (docs/specs/spec-58-geocoding-foundation.md, "Fase 2")
--
-- ============================================================================
-- SPLIT FROM THE DDL MIGRATION -- DO NOT MERGE BACK TOGETHER
-- ============================================================================
--
-- `centroid_lat`/`centroid_lng` are added by
-- `20261011000001_spec58_comuna_centroids.sql` (DDL only). This file is
-- the 347-row seed, deliberately alone in its own migration with no DDL
-- at all: `scripts/check-migration-safety-rule1.mjs` only inspects a
-- file's UPDATE/backfill shape when that SAME file also contains DDL
-- (`findRule1Violations` returns `[]` immediately otherwise --
-- check-migration-safety-rule1.mjs:242). A plain top-level `UPDATE ...
-- FROM (VALUES ...)` here, with no `ALTER TABLE`/`CREATE TABLE` anywhere
-- in this file, is therefore never evaluated against rule 1's DDL+backfill
-- mix at all -- not narrowly exempted, not passing by a regex detail:
-- the check for it never runs. `20260909000001_spec79_loaded_route_id.sql`
-- solves the same rule with a different, function-wrapped shape because
-- ITS backfill is genuinely risky at production scale and must be
-- deferred to a human running it by hand after measuring; this seed is
-- 347 literal, already-known rows against an already-indexed primary key
-- on a fixed reference table, so it runs automatically, right here, with
-- no wrapper and no deferred step.
--
-- 20261011000002 is Fase 3's `geocode_cache` constraints migration
-- (already merged) -- this file is 20261011000003, not 20261011000002,
...
--
-- Verification pass, run three times (first draft, review round 1, review
-- round 2):
--
...
--      round 1), confirming 346/347 land inside the correct comuna's
...
--      mode passes 1-3 cannot see: `ST_PointOnSurface` is correct *for
--      the polygon* but can land 20-142km from any road, building or
--      named place when the polygon interior is open water or high
--      cordillera (real for Aysén and Magallanes region comunas covering
--      fjords and icefields). Ten rows were replaced with the town/
--      village/hamlet node instead -- see "TOWN-NODE REPLACEMENTS" below.
--      This is a second, distinct extraction method from the general rule
--      above, documented explicitly per-row for exactly that reason: a
--      uniform "one source, one method" claim would not survive an audit
--      of these ten rows otherwise.
--
...
--      further into the uninhabited territorial claim, and the spec was
...
-- Plain top-level UPDATE ... FROM VALUES, matched on codigo_cut so row
-- order here carries no meaning and a future re-seed can freely reorder.
-- 347 pairs, exactly matching the 347 rows seeded by
-- 20260321000001_chile_comunas_normalization.sql:43-407. No DDL in this
-- file -- see this file's header for why that is what makes this
-- compliant with rule 1, not a wrapper function or a deferred manual step.
