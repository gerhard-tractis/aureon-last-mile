-- =============================================================================
-- Pre-Ruta hands out orders that are already loaded, or already at DispatchTrack
-- =============================================================================
-- get_pre_route_snapshot (latest def: 20260817000003_fix_pre_route_sectorizado_cohort.sql)
-- excludes an order from the unrouted cohort when it has a dispatch on a route
-- whose status is IN ('draft', 'planned', 'in_progress') — the vocabulary from
-- before spec-70's state machine.
--
-- Phase 1 (20260825000002) remapped every route that meant "sent to DT" under
-- the old vocabulary from `planned` to `dispatched`. Phase 3 makes `loading`
-- and `loaded` routine states a route sits in for the whole loading pass.
-- None of `dispatched`, `loading`, `loaded` are in the exclusion list above, so
-- as soon as this ships an order that is mid-load, already sealed, or already
-- accepted by DispatchTrack reappears in Pre-Ruta as unrouted — and can be
-- seeded onto a second route. A package on two trucks.
--
-- FIX: exclude on every status a route is "active" under, matching
-- lib/dispatch/types.ts's ACTIVE_ROUTE_STATUSES exactly — the same set that
-- ownsTheOrder() (scan-validator.ts) already uses to decide whether a dispatch
-- row still owns its order. `completed` and `cancelled` are deliberately absent
-- from both: that is what lets a failed delivery return through `retorno_hub`
-- (spec-43) and be routed again.
--
-- Template: 20260817000003_fix_pre_route_sectorizado_cohort.sql, per the
-- CLAUDE.md rule that CREATE OR REPLACE rewrites start from the latest
-- definition. Only the routed_ids status list and its comment differ.
-- =============================================================================
