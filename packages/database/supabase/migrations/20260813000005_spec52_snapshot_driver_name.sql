-- =============================================================================
-- spec-52 Task 12 — the reception snapshot returns the driver's name
-- =============================================================================
-- RouteReceptionHeader.tsx has rendered `snapshot.route.driver_name` since
-- spec-47. `pickup_routes` HAS NO SUCH COLUMN — it carries `driver_id` only —
-- and this RPC built its `route` node from `to_jsonb(pr.*)`, so the key was
-- never emitted. `{driverName && (...)}` in the header then swallowed the
-- entire driver line: no crash, no warning, just a blank truck row on every
-- reception screen in production for months.
--
-- That makes three defects in this one RPC (the #398 key renames, `pk.id AS
-- package_id`, and now this). All three shared one cause: the contract test's
-- presence list for the `route` node did not name the field the component
-- reads. The fix is therefore two-sided — the join below, and `driver_name`
-- added to that presence list in
-- packages/database/supabase/tests/route_reception_snapshot_contract.sql.
-- Presence alone would not have caught a join wired to the wrong column, so
-- the test asserts the VALUE too.
--
-- TEMPLATED ON THE LATEST DEFINITION: 20260813000004_spec52_snapshot_vehicle_plate.sql
-- (NOT the spec-47 original 20260625000001:493 and NOT 20260813000003 — either
-- would silently revert the plate join, or the plate join AND the #398 key
-- names, on top of each other. CLAUDE.md's CREATE OR REPLACE rule: always the
-- latest.) Everything below is byte-for-byte 20260813000004 except the
-- `v_route` SELECT, which gains a second LEFT JOIN and the `driver_name` merge
-- alongside the existing `plate` one.
--
-- LEFT JOIN, not JOIN — same reasoning as the vehicle join it sits beside: a
-- route whose user row was removed must still be receivable. `driver_name`
-- then comes back NULL, which is exactly what
-- `RouteReceptionRouteHeader.driverName: string | null` declares and what the
-- header's guard already handles.
--
...
BEGIN
  ...
  -- The ONLY change from 20260813000004: the users join and the `driver_name`
  -- key. Operator scoping, the deleted_at filter and the not-found raise below
  -- are unchanged. The users join needs no operator predicate of its own —
  -- `pr.operator_id = v_operator` already fences the driving row, and
  -- `pickup_routes.driver_id` is FK-bound to a user of that same operator.
