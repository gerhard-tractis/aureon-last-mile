...
19. Verificación responsive: las tres columnas colapsan a pestañas bajo 1024px (regla del handoff, *Interactions & Behavior*). **Verificado — parcialmente cierto, con un hallazgo.**
    ...
    **Lo que existe de verdad, contra el código:** `PreRouteBoard.tsx` no monta ningún `Tabs`/`TabsList` ni rama con `useIsBelowLg()` alrededor de sus tres columnas. Grep sobre el componente y sus tres hijos (`UnroutedColumn`, `RoutePlanCanvas`, `RouteDraftPanel`) no encuentra ni una referencia a pestañas ni al hook. Lo que hay, sin cambios desde `spec-54` fase 4.2 — el propio docblock del componente ya lo decía: «Below 1024px the columns stack» — es una única grilla CSS (`grid ... lg:grid-cols-[330px_1fr_322px]`), una columna por default y tres recién a partir de `lg` (1024px, el mismo umbral que resuelve `useIsBelowLg`, pero alcanzado aquí por clases `lg:` de Tailwind, no por el hook). Bajo 1024px **las tres columnas siguen montadas y visibles**, apiladas verticalmente — no hay picker que oculte dos de tres.

    **El hallazgo:** el texto de este ítem («colapsan a pestañas») viene literal del handoff (`design_handoff_aureon_rebrand/README.md`, sección *Interactions & Behavior*), que ya no existe en este repo para cotejarlo — no se puede confirmar si describía de verdad una interacción de pestañas o si es la misma clase de arrastre de numeración/redacción que la decisión 2 de este spec ya diagnosticó para otro dato del mismo handoff. Lo cierto es que **no hay pestañas** — el patrón real es apilar, y es el mismo que ya documentan `spec-54` (este componente), `spec-62` (Recepción móvil) y `spec-65` (Pedidos) para layouts equivalentes de este repo. Decidir si el ítem 19 debía pedir pestañas de verdad (trabajo nuevo, no construido aquí) es una decisión de producto que no se toma en esta fase — no se implementó ninguna pestaña para hacer pasar el ítem tal como estaba escrito.

    Pinneado en `PreRouteBoard.test.tsx` (dos tests): (1) las tres columnas se montan siempre, sin `tablist`/`tab` alguno en el árbol de accesibilidad; (2) la grilla lleva la clase `lg:grid-cols-[330px_1fr_322px]` sin una utilidad `grid-cols-1` explícita — el apilamiento bajo `lg` es el default implícito de CSS Grid, tal como describe el comentario del componente. Este test fija el comportamiento **real** para que no regresione en silencio; no certifica el ítem 19 tal como está redactado.
