...
- **M-4 (mayor).** B2 (ronda 1) cerró el 42501 con `uploaded_by: null`, pero abrió un hueco de trazabilidad: `manifest_documents.uploaded_by` es el respaldo de "quién fotografió", y con `null` fijo esa información no quedaba en ningún sitio para la ruta offline (el `userId` vive sólo en IndexedDB y `purgeConfirmed` lo borra), dejando la columna medio poblada — no-nula online, siempre nula offline. **Implementado:** `sendManifestPhoto` resuelve `auth.uid()` de la SESIÓN QUE DRENA (`supabase.auth.getUser()`) en el momento del envío — la misma fuente que compara la policy — y sólo cae a `null` si de verdad no hay sesión resoluble.
