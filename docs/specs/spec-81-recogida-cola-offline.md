...
- [ ] El drenador pasa el token que `claimPending` devuelve a `markFailed`/`markSent` como `claimedAt` (implementado en fase 1, ronda 4 de review, M1 — ver checklist de esa fase) — sin esto la protección existe en el contrato pero ningún llamador la usa.
...
- [ ] **`getPendingPickupCount` cuenta también `dead` (ronda 4 de review de fase 1, N6).** Hoy sólo cuenta `pending`; una entrada `sending` huérfana o `dead` nunca se envió y el chip marcaría 0 mientras algo sigue sin llegar al servidor.
