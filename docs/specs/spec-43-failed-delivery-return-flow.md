...
> **Fix (spec-77, `20260908000002`):** `get_pre_route_snapshot` (spec-77 phase 1b, `20260908000001`) added a global predicate that hides any package with a genuine load fact (`loaded_at IS NOT NULL AND load_inferred = false`) from Pre-Ruta, regardless of order — needed so a `force_split` order's already-shipped half does not reappear as "ready". Nothing on THIS flow ever cleared that fact: a box that travelled on a route, failed delivery, and came back kept its original `loaded_at` from the completed route, and once re-dock-scanned back to `sectorizado` it was hidden from Pre-Ruta **permanently** — dead re-route flow for exactly the case this spec exists to support. The dock-scan path (`SCANNABLE_STATUSES = ['en_bodega', 'sectorizado']`, `dock-scan-validator.ts`) means a `retorno_hub` package can only ever reach `sectorizado` again by first passing through THIS RPC (`retorno_hub` → `en_bodega`), which makes it the single, guaranteed choke point on the return path — the same role the two "remove from plan" endpoints already play for the load-planning side (`routes/[id]/packages/[pkgId]/route.ts`, `routes/[id]/route.ts`). The `UPDATE packages` above now also clears `loaded_at`/`loaded_by`/`load_inferred` — a box coming back from a failed delivery is not loaded on anything. `process_failed_delivery` and the dock-scan trigger were deliberately left untouched: `ready_pkgs` already excludes `retorno_hub`/`en_bodega` by status, so a stale `loaded_at` is inert until this RPC runs. Regression test: `packages/database/supabase/tests/spec77_retorno_hub_load_fact.test.sql`.
