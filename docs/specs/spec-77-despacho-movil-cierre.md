...
_2026-09-05, PR #628._

19. `[done]` `npx vitest run --pool=forks`: 5732 passed, 45 skipped, 0 fallos reales (1 flake preexistente sin relación al módulo de despacho, confirmado en verde aislado). `npx tsc --noEmit` y `npm run lint` limpios. Mutation-test manual sobre el seam nuevo (`dispatch-test-hooks.ts`), ambas ramas mueren, revertidas.
20. `[done]` `apps/frontend/e2e/despacho-close-dispatch.spec.ts` — namespace propio (`despacho-close-fixture.ts`/`despacho-close-journey.ts`, `PREFIX 'E2E77'`, grepeado contra el resto de `e2e/` antes de usarlo, no-colisionante con `E2E52`/`E2E76`/`E2E78`), no sobre `despacho-fixture.ts` de `spec-76` — reutilizar ese namespace habría dejado el `teardown()` de esta suite compitiendo con una ruta de `spec-76` todavía en carga, la misma trampa que `playwright.qa.config.ts` ya documenta entre spec-52 y spec-76. "Route H" cubre el camino completo cargar → cerrar (forzado, con una orden partida) → despachar.
21. `[done, con una desviación honesta]` DispatchTrack se mockea a nivel de red, pero **no** con `context.route()` interceptando `**/activationcode.dispatchtrack.com/**` como decía este ítem originalmente — se verificó que ese mock nunca se ejercita de verdad: el POST a DT (`createDTRoute`) corre en el servidor (la ruta API de Next.js), y `context.route()` sólo intercepta las peticiones que hace el propio navegador. El mecanismo real es `infra/supabase-qa/dispatchtrack-mock/` — un servidor HTTP standalone al que se apunta `DISPATCHTRACK_BASE_URL` en QA, en vez del tenant real. Nunca se despacha de verdad contra DT.

    **Hallazgo de seguridad, corregido en la misma tarea:** antes de este cambio, `.env.qa` tenía un `DISPATCHTRACK_API_KEY` real contra el tenant real de Transportes Musan **sin ningún** `DISPATCHTRACK_BASE_URL` — cualquier despacho disparado en QA, manual o automático, golpeaba producción. Corregido apuntando `DISPATCHTRACK_BASE_URL` al mock.
22. `[done, con una desviación honesta]` Los tres caminos están cubiertos, cada uno en su propia ruta seeded ("Route R", "Route H", "Route L"): DT rechaza (`DT_API_ERROR`, `2k` primer estado) · DT acepta (camino feliz, `2l`) · **DT acepta y la escritura local falla** (`DT_ACCEPTED_LOCAL_FAILED`, `2k` segundo estado, reintento vía *Completar*). El tercer camino **no tiene ninguna forma legítima de reproducirse por seed de datos** sin debilitar la aserción (weakening explícitamente prohibido por este mismo ítem) o correr una carrera de timing genuina y por lo tanto inestable — es una aceptación real de DispatchTrack compitiendo con un fallo de base local real. Se agregó un seam QA-only, doble-gateado por diseño (`ALLOW_E2E_TEST_HOOKS` en el entorno + un header de request que sólo el propio test agrega, `apps/frontend/src/lib/dispatch/dispatch-test-hooks.ts`) que dispara el fallo recién **después** de que `external_route_id` ya se persistió de verdad — exactamente la ventana que la Fase 0 de `spec-79` describe, no un atajo que la evite. El E2E asegura además que el reintento **no** crea una segunda ruta en DT, contra el propio log de llamadas del mock (`/__test__/create-calls`) — el punto entero de este ítem.
23. `[done]` `Route H` del mismo spec: una orden partida a propósito (2 bultos, 1 escaneado), fuerza el cierre (`2i`), despacha, y verifica por consulta directa a la base que el bulto nunca escaneado **no** queda `en_ruta` mientras los genuinamente cargados sí.
24. `[done]` **Corrida real confirmada post-merge, PR #628, run [33987117912](https://github.com/gerhard-tractis/aureon-last-mile/actions/runs/33987117912), job "E2E against QA".** El primer intento de `deploy-qa` falló por un error propio de esta tarea (ver autocrítica abajo), corregido en caliente; el segundo intento (`gh run rerun --failed`) corrió los 23 tests de `e2e-qa` con 1 worker real contra QA. Leyendo el LOG real, no el check verde (`continue-on-error: true`): **los tres tests de `despacho-close-dispatch.spec.ts` pasaron** (Route H 13.6s, Route R 11.3s, Route L 6.7s) — primeros en el orden alfabético del runner. De los 23 tests totales, **22 pasaron y 1 falló**: `despacho-tablet-dock.spec.ts` (`spec-78`, timeout de 300s esperando un botón) — **no relacionado con este PR** (archivo/fixture/namespace de otro spec, no tocado acá). No se investigó más a fondo por estar fuera de alcance de spec-77/79.

    **Autocrítica — un error propio, encontrado y corregido en la misma tarea.** El primer intento de `deploy-qa` post-merge falló: `git reset --hard` no pudo `unlink` `infra/supabase-qa/dispatchtrack-mock/server.mjs` porque ese archivo (y su directorio) habían quedado con dueño `root` — se crearon vía `scp`/`ssh` como root en vez de como `aureon`, el usuario bajo el que corre el runner y todo el checkout de `~/aureon-qa`. Corregido: `chown -R aureon:aureon` sobre ese directorio, el mock reiniciado como `aureon`, y el deploy reintentado con éxito. Lección para el ítem 25 de abajo: cualquier instalación manual futura en el VPS debe correr como `aureon`, nunca como `root`, dentro de un checkout que `deploy-qa.sh` gestiona.
25. `[pending, requiere acceso interactivo al VPS]` Instalar `aureon-dt-mock-qa.service` como unidad systemd real (ver el archivo y su comentario de instalación, y la autocrítica del ítem 24 sobre el usuario correcto) para que el mock sobreviva un reinicio del VPS.
