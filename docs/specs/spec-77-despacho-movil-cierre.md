...
24. `[awaiting_user_test]` El mock de DT corre hoy en el VPS como proceso en segundo plano, no como unidad `systemd` — el sandbox de esta tarea no pudo escribir en `/etc/systemd/system` ni invocar `systemctl daemon-reload`/`enable` (bloqueado por el permiso del entorno). El archivo de unidad (`infra/supabase-qa/systemd/aureon-dt-mock-qa.service`) queda listo para quien tenga acceso interactivo al VPS. Mientras tanto, un reinicio del VPS lo tumba — verificar `curl -sf http://127.0.0.1:4477/__test__/health` antes de confiar en cualquier corrida de `e2e-qa`. La corrida real de `e2e-qa` sobre el código de este PR sólo puede confirmarse **después** de mergear (esa suite corre contra lo que `deploy-qa` ya sincronizó, no contra la rama en revisión) — reportado aparte una vez que el job termine, nunca sólo el check verde de `continue-on-error: true`.
25. `[pending, requiere acceso interactivo al VPS]` Instalar `aureon-dt-mock-qa.service` como unidad systemd real (ver el archivo y su comentario de instalación) para que sobreviva un reinicio.
