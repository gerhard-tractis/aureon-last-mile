...
**Pendiente aparte del checklist de arriba — con dueño: lo cierra quien mergee el segundo de #712 (spec-81 fase 5) y #713 (esta fase), sobre el código real, no sobre el orden que este párrafo pueda anticipar (M3, review del PR #712; nota de coordinación: este párrafo vive separado de la lista de checkboxes a propósito, para no chocar con la línea que #713 modifica).** Si `#713` conecta la captura (`5g`/`5h`) a `useUploadManifestDocument` — la ruta ONLINE, sin salida sin señal — antes de que `#712` (que construye la ruta offline completa para fotos: blob a IndexedDB, subida diferida con reintento, huérfano imposible, `enqueueManifestPhoto`/`sendManifestPhoto` en `lib/offline/photos.ts`) aterrice, queda un recableado pendiente, no una decisión nueva: cambiar la llamada de `useUploadManifestDocument` a `enqueueManifestPhoto` es lo que hace verdad «Las fotos también» (`5f`) en el código que corre, no sólo en la infraestructura que exista para recibirlo. Si el orden de merge resulta el inverso, quien implemente esta fase ya tiene `enqueueManifestPhoto` disponible y debe llamarlo directamente — no hay recableado que hacer.
