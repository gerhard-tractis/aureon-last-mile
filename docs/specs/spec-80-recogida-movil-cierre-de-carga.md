...
**Pendiente aparte del checklist de arriba — recableado, no elección inicial (M3, review del PR #712 de spec-81 fase 5; nota de coordinación con #713, que aterriza antes: este párrafo vive separado de la lista de checkboxes a propósito, para no chocar con la línea que #713 ya marca `[x]`).** `#713` conectó la captura (`5g`/`5h`) a `useUploadManifestDocument` — la ruta ONLINE, sin salida sin señal. spec-81 fase 5 (mergeada después) construyó la ruta offline completa para fotos (blob a IndexedDB, subida diferida con reintento, huérfano imposible, `enqueueManifestPhoto`/`sendManifestPhoto` en `lib/offline/photos.ts`) precisamente para cerrar ese hueco, pero no tocó `ManifestPhotoStrip.tsx` ni esta pantalla a propósito — coordinación explícita para no pisar esta fase 4 mientras estaba en vuelo. Queda un recableado pendiente, no una decisión nueva: cambiar la llamada de `useUploadManifestDocument` a `enqueueManifestPhoto` es lo que hace verdad «Las fotos también» (`5f`) en el código que corre, no sólo en la infraestructura que ya existe para recibirlo.
