...
**Esto también cambia el escritorio** (`RouteBuilder`'s `ScanZone` usa el mismo validador — correcto e intencional, ambas superficies deben rechazarlo) y **el conteo de `packagesTotal`/`boxesTotal`** en todo lugar que comparte `DISPATCHABLE_STATUSES` (`useRoutePackages.ts`, `crew-board.ts`'s `aggregateBoxesByRoute`, `loading-monitor-aggregate.ts`): un paquete `en_bodega` sin `loaded_at` ya no cuenta en el total, por la misma razón que un `dañado`/`retenido` nunca contó — contarlo inflaba un total que la ruta nunca podía alcanzar. `ON_ANDEN_STATUSES` (`anden-status.ts`) pasó a ser un alias de `DISPATCHABLE_STATUSES` en vez de una copia a mano — los dos conjuntos convergieron, así que la figura "EN EL ANDÉN" (`2c`) y lo que el bucle de escaneo acepta (`2e`) ahora están de acuerdo, cerrando la inconsistencia que la tarea 2 dejó abierta.

**Caveat de despliegue — anotado, no ejecutado.** Esto altera comportamiento en vivo: un paquete que hoy el andén puede cargar empezará a rechazarse. Antes de que esto llegue a producción, alguien debe contar cuántos paquetes se escanearon a una ruta estando en `en_bodega` en los últimos 30 días. Si es ~0, es un cierre de brecha sin costo; si no lo es, algún flujo real depende de eso y hace falta una ruta de anulación (override) en vez de un bloqueo sin salida. Esta consulta **no se ejecutó** contra producción como parte de esta tarea — es un chequeo pre-despliegue pendiente, no un hallazgo verificado.
