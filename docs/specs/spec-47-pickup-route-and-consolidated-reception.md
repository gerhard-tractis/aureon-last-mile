...
Three PRs, executed in order. Each is independently green-CI before the next starts. TDD throughout: write the failing test, ship the minimal change, refactor.

### PR #1 — Migration + RPCs (database only)

Branch: `feat/spec-47-migration`

1. **pgTAP red** — write `spec47_pickup_routes_rls.sql`, `spec47_single_active_route_per_driver.sql`, `spec47_close_route_creates_route_reception.sql`, `spec47_close_route_zero_packages_fails.sql`, `spec47_reception_scan_increments_count.sql`, `spec47_complete_route_cascades_manifest_status.sql`, `spec47_cancel_route_detaches_manifests.sql`, `spec47_migration_invariants.sql`. All red.
2. **Enum + tables** — migration file creates `pickup_route_status_enum`, `pickup_routes`, `route_receptions`, `manifests.pickup_route_id`.
3. **Indexes + RLS + audit + grants** — partial unique index for single-active, tenant policies, audit triggers, set_updated_at, grants.
4. **Triggers** — `trg_pickup_routes_set_manifest_reception_status`, `trg_route_receptions_status_sync`, `trg_reception_scans_route_received_count`.
5. **RPCs** — `start_pickup_route`, `add_manifest_to_route`, `close_pickup_route`, `cancel_pickup_route`, `get_route_reception_snapshot`, `complete_route_reception`.
6. **Backfill** — synthetic `PR-LEGACY-*` routes for in-transit manifests, 1:1 `hub_receptions` → `route_receptions` copy, FK repoint.
7. **Drop `hub_receptions`** and flip `reception_scans.reception_id` FK.
8. **All pgTAP green** + commit + PR + auto-merge.

### PR #2 — Driver-side (pickup)

Branch: `feat/spec-47-pickup-driver-ui`

1. **Hook tests red** — `useActivePickupRoute`, `useStartPickupRoute`, `useAddManifestToRoute`, `useClosePickupRoute`.
2. **Hooks implemented** against the RPCs from PR #1.
3. **Component tests red** — `ActiveRouteBanner`, `StartRouteButton`, `RouteManifestList`, `AddManifestSheet`, `CloseRouteButton`, `RouteQRView`.
4. **Components implemented.**
5. **Page tests red** — `/app/pickup` modifications, `/app/pickup/route/active`, `/app/pickup/route/[routeId]/qr`.
6. **Pages implemented.**
7. **Delete** `apps/frontend/src/app/app/pickup/handoff/[loadId]/` (page + test), `apps/frontend/src/hooks/reception/useQRHandoff.ts` (hook + test), `apps/frontend/src/components/reception/QRHandoff.tsx` (component + test).
8. **Playwright** `spec47-pickup-route-end-to-end.spec.ts` green.
9. **All green** + commit + PR + auto-merge.

### PR #3 — Hub-side (reception)

Branch: `feat/spec-47-reception-hub-ui`

1. **Hook tests red** — `useRouteReceptionSnapshot`, `useReceptionScan` (modified), `useCompleteRouteReception`.
2. **Hooks implemented.**
3. **Component tests red** — `IncomingRoutesList`, `RouteQRScannerEntry`, `ConsolidatedScanList`, `RouteReceptionHeader`, `FinalizeReceptionButton`.
4. **Components implemented.**
5. **Page tests red** — `/app/reception` modifications, `/app/reception/route/[routeId]`.
6. **Pages implemented.**
7. **Delete** `apps/frontend/src/app/app/reception/scan/[receptionId]/` (page + test). Refactor `ReceptionCard` to render a route, not a manifest.
8. **Playwright** `spec47-consolidated-reception.spec.ts` green — explicitly scans packages from two manifests in arbitrary order.
9. **All green** + commit + PR + auto-merge.

### Done condition

User confirms in production-staging that:
- A driver can start one route, add ≥2 manifests, verify packages in each, close the route, and a single QR appears.
- A receptionist scans that QR (or types the code) and sees every order from both manifests grouped, can scan any package in any sequence, and finalize when complete.
- Legacy in-transit manifests from before the migration are still reachable via their synthetic `PR-LEGACY-*` code and complete reception correctly.

After user confirmation, `Status:` flips to `completed`.
