...
- **El veto del step, no sólo del job.** `check-deploy-gating.mjs` ahora exige que `e2e-qa` tenga exactamente un step cuyo `run:` sea, como línea lógica (uniendo continuaciones con `\`), exactamente `bash scripts/check-quarantine.sh <quarantine.json> <report.json>` — sin flags (`--validate-only` incluido), sin operadores de control alrededor, sin `set +e` ni `exit` en ningún otro punto del mismo `run:`, y sin `shell:` alternativo (`bash {0}` tira el `-e`/`pipefail` por defecto). Es una aserción positiva de la única forma aceptada, no una lista negra: un fix round 3 mostró ocho vectores que la lista negra original (`|| true`, `|| :`, comentarlo, `echo`) dejaba pasar con exit 0, entre ellos `--validate-only` — un flag real que esta misma fase introdujo — y un step señuelo anterior que `steps.find` tomaba como el real. La lista cerrada era esa lista, no la clase; la aserción positiva no tiene «vector nueve» que se le escape.
