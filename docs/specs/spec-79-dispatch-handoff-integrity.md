...
- **Rutas `loaded` legadas nunca reescaneadas (Fase 1c, F1 revisión #2).** El requisito `load_inferred = false` significa que una ruta sellada antes de spec-74 y jamás reescaneada produce `packages_dispatched: 0` para siempre — no es un bug, es la contraparte deliberada de negarse a marcar `en_ruta` un bulto que nunca salió del andén. Si en producción existen rutas así aún sin despachar, necesitan reconciliación manual puntual (repasar, reescanear o descartar), no otro cambio de código. Se desconoce cuántas son — la misma medición pendiente del punto 7 de Fase 1 lo cubre.
