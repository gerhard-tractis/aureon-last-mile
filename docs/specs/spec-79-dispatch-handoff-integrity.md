...
- **pgTAP no ejecutado contra el contenedor compartido.** `spec79_phase4_dispatch_claim.test.sql` se escribió y se leyó con cuidado (mismo estilo que `spec79_loaded_route_id.test.sql`), pero el contenedor `spec52-pg` estaba corriendo y no se pudo confirmar que ningún otro agente lo estuviera usando en paralelo — mismo riesgo documentado en Fase 1f. Queda pendiente para quien tenga la ventana de correrlo.
- **El índice `routes_one_vehicle_per_day` no se auto-crea si producción ya tiene conflictos** — ver el `RAISE NOTICE` de su propia migración. Nadie ha medido si existen.
